...
export const userDto = z.object({ name:short.min(2),email:z.string().email().max(254),roleId:id,phone:short.optional(),whatsapp:short.optional(),whatsappOptIn:z.boolean().default(false),password:z.string().min(12).max(128),clientId:id.optional(),avatarUrl:z.string().max(7000000).optional().nullable(), permissions:z.array(z.object({key:id,allowed:z.boolean()}).strict()).max(100).optional() }).strict();
...
export const shootDto=z.object({scheduledAt:date,location:short.min(1),shotList:text.min(1),props:text.optional(),people:short.optional(),orientation:z.enum(['Portrait','Landscape','Square']),durationMinutes:z.number().int().min(1).max(1440),referenceUrl:webUrl.optional(),notes:text.optional()}).strict();
export const actionDto=z.object({action:z.enum(actions),revision:z.number().int().min(0),comment:text.optional(),timestampComments:z.array(z.object({timestamp:z.string().regex(/^\d{1,3}:\d{2}$/),comment:text.min(1)}).strict()).max(50).optional(),referenceUrls:z.array(webUrl).max(20).optional(),rawUrl:drive.or(webUrl).optional(),publishedUrl:webUrl.optional(),externalPostId:short.optional(),scheduledAt:date.optional()}).strict();
export const versionDto=z.object({url:drive.or(webUrl),notes:text.optional(),revision:z.number().int().min(0)}).strict();
export const taskDto=z.object({title:short.min(3),description:text.optional(),clientId:id,contentId:z.number().int().positive().optional(),assigneeId:id,priority:z.enum(['LOW','MEDIUM','HIGH','URGENT']),startsAt:date.optional(),dueAt:date,driveUrl:webUrl.optional()}).strict();
...
export const commentDto=z.object({body:text.min(1),timestamp:short.optional(),referenceUrl:webUrl.optional(),clientVisible:z.boolean().default(false)}).strict();
export const driveDto=z.object({clientId:id,contentId:z.number().int().positive().optional(),taskId:id.optional(),revisionId:id.optional(),title:short.min(1),url:drive.or(webUrl),category:short.min(1),clientVisible:z.boolean().default(false)}).strict();
